...
for (i in seq_along(file)) {
	command = paste0("module load PLINK/1.90-beta5.3; plink --bfile /ysm-gpfs/pi/zhao/gz222/UKB_simulate/genotype/validate/validate_10K --score ",file[i]," 1 2 3 header --out ",file[i] )
	if (length(readLines(file[i])) != 681828) {next}
...
command = paste0("module load PLINK/1.90-beta5.3; plink --bfile /ysm-gpfs/pi/zhao/gz222/UKB_simulate/genotype/test/test_10K --score ", file[idx], " 1 2 3 header --out ../predict/lassosum/sim_",j)
