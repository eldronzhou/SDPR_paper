...
for (j in 1:3) {
        for (i in 1:10) {
                pheno = read.table(paste0("../../../phenotype/Scene1B/h2_0.5/test/sim_",i,".phen"), header=F, stringsAsFactors=F)
...
tiff("Scenario_1B.tiff", units="in", width=6, height=4, res=300)
...
ggplot(data=res, aes(x=sample_size, y=R2,fill=method)) + 
        ...
        ggtitle("Scenario 1B") + theme(plot.title=element_text(hjust = 0.5),
