...
SDPR = read.table("./SDPR/res.profile.gz", header=T, stringsAsFactors=F)
...
PRS_cs = read_profile("./PRS_CS_MHC/")

ldpred = read_profile("./LDpred_MHC//")
...
dat = dat[!rm_idx, c("f.eid","IBD","f.22001.0.0","f.21003.0.0",paste0("PC",1:10))]
colnames(dat) = c("ID","IBD","sex","age", paste0("PC",1:10))
a = which(dat$IBD == 1)
b = which(dat$IBD != 1)
...
get_res = function(dat, i=-1) {
        ...
        if (i == -1)
                lm_fit_full = glm(IBD~SCORE, data=dat,
                                family=binomial(link="logit"))
        else
                lm_fit_full = glm(IBD~dat[,i], data=dat,
                                  family=binomial(link="logit"))
...
SDPR = read.table("./SDPR/res_MHC.profile", header=T, stringsAsFactors=F)
...
tiff("IBD.tiff", units="in", width=6, height=4, res=300)
ggplot(res3, aes(x=method, y=mean, fill=method)) + 
        geom_bar(stat="identity") + 
        geom_errorbar(aes(x=method, ymin=mean-sd, ymax=mean+sd), width=.5) + 
        ggtitle("IBD") + coord_cartesian(ylim=c(0.5,.75)) + ylab("AUC") + theme_bw(10) +
