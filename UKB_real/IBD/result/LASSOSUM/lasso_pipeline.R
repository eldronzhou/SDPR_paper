...
ref.bfile <- "/ysm-gpfs/pi/zhao/gz222/1000g_phase3/genotype_1KG_eur_SNPmaf5/hm3/eur_SNPmaf5"

LDblocks <- "EUR.hg19"

bim = read.table(paste0(ref.bfile, ".bim"), header=F, stringsAsFactors=F)
#idx = match(ss$SNP, bim[,2])
#idx2 = which(ss$A1 == bim[idx,6] & ss$A2 == bim[idx,5])
#ss[idx2]$b = -ss[idx2]$b
#print(paste0("flipping ", length(idx2)))

cor <- p2cor(p=ss$P, n=32372, sign=ss$log_odds)
