...
dat = dat[!rm_idx, c("f.eid","BP","f.22001.0.0","f.21003.0.0",paste0("PC",1:10),"SCORE")]
colnames(dat) = c("ID","BP","sex","age", paste0("PC",1:10),"SCORE")
a = which(dat$BP == 1)
b = which(dat$BP != 1)
...
get_res = function(dat, i=-1) {
        ...
        if (i == -1) 
                lm_fit_full = glm(BP~sex+age+SCORE+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10, data=dat,
                                family=binomial(link="logit"))
        else 
                lm_fit_full = glm(BP~age+dat[,i]+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10, data=dat,
                                  family=binomial(link="logit"))
...
tiff("BP2.tiff", units="in", width=6, height=4, res=300)
ggplot(res2, aes(x=method, y=R2)) + geom_boxplot(outlier.shape=NA, aes(fill=method)) + 
        geom_jitter(size=.5) + ggtitle("Bipolar") + 
