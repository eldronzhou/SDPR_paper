...
dat = dat[!rm_idx, c("f.eid","T2D","f.22001.0.0","f.21003.0.0",paste0("PC",1:10))]
colnames(dat) = c("ID","T2D","sex","age", paste0("PC",1:10))
a = which(dat$T2D == 1)
b = which(dat$T2D != 1)
...
get_res = function(dat, i=-1) {
        ...
        if (i == -1)
                lm_fit_full = glm(T2D~SCORE, data=dat,
                                family=binomial(link="logit"))
        else
                lm_fit_full = glm(T2D~dat[,i], data=dat,
                                  family=binomial(link="logit"))
...
tiff("T2D.tiff", units="in", width=6, height=4, res=300)
ggplot(res3, aes(x=method, y=mean, fill=method)) + 
        geom_bar(stat="identity") + 
        geom_errorbar(aes(x=method, ymin=mean-sd, ymax=mean+sd), width=.5) + 
        ggtitle("T2D") + coord_cartesian(ylim=c(0.5,.75)) + ylab("AUC") + theme_bw(10) +
