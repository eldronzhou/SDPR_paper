...
get_cor = function(dat, i=-1) {
        if (i == -1)
                a = summary(lm(height~sex+age+SCORE+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10, data=dat))$residuals
        else
                a = summary(lm(height~sex+age+dat[,i]+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10, data=dat))$residuals
        b = summary(lm(height~sex+age+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10, data=dat))$residuals
        1 - sum(a^2)/sum(b^2)
}
...
ggplot(res2, aes(x=method, y=R2)) + geom_boxplot(outlier.shape=NA, aes(fill=method)) + 
        geom_jitter(size=.5) + ggtitle("Height") + ylim(c(.2,.28)) + ylab("R2") + 
