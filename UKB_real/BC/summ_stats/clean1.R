a = read.table("BCAC_BC_2017.txt", header=T, stringsAsFactors=F)
a[,2] = sapply(strsplit(a[,2], ":"), function(x) x[1])
idx = grep("rs", a[,2])
a$N = 4*122977*105974/(122977+105974)
a = a[,c(2,6,5,8,10,11)]
colnames(a) = c("SNP","A1","A2","log_odds","P","N")
write.table(a[idx,], file="clean1.txt", row.names=F, col.names=T, quote=F, append=F)
