a = read.table("jointGwasMc_HDL.txt", header=T, stringsAsFactors=F)
a = a[!duplicated(a$SNP_hg19),]
rm_idx = which((a$be/a$se)^2 > 80)
a$P = 2*pnorm(-abs(a$beta/a$se))

write.table(a[-rm_idx,c("rsid","A1","A2","beta","P","N")], file="clean1.txt", row.names=F, col.names=T, quote=F, append=F)
...
# prepare for gctb
d = a[a$rsid %in% b$SNP,]
d = d[,c("rsid","A1","A2","Freq.A1.1000G.EUR","beta","se","P-value","N")]
