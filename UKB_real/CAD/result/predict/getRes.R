...
pheno = read.table("./eur_case_for_prs_comparison.tsv", header=T, stringsAsFactors=F)
bd = inner_join(bd, pheno, by=c("f.eid"="eid"))

dat = inner_join(bd, SDPR, by=c("f.eid"="FID")) %>% 
        inner_join(pcs, by=c("f.eid"="FID"))

rm_idx = (is.na(dat$CAD) | is.na(dat$f.22001.0.0) | is.na(dat$f.21003.0.0)) 
...
dat = dat[!rm_idx, c("f.eid","CAD","f.22001.0.0","f.21003.0.0",paste0("PC",1:10),"SCORE")]
colnames(dat) = c("ID","CAD","sex","age", paste0("PC",1:10),"SCORE")
a = which(dat$CAD == 1)
b = which(dat$CAD != 1)
...
get_res = function(dat, i=-1) {
        lm_fit_base = glm(CAD~sex+age+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10, data=dat,
                          family=binomial(link="logit"))
        l0 = as.numeric(logLik(lm_fit_base))
        if (i == -1)
                lm_fit_full = glm(CAD~sex+age+SCORE+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10, data=dat,
                                  family=binomial(link="logit"))
        else
                lm_fit_full = glm(CAD~sex+age+dat[,i]+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10, data=dat,
                                  family=binomial(link="logit"))
        l1 = as.numeric(logLik(lm_fit_full))
        R2 = (1 - exp(2/nrow(dat)*(l0-l1))) / (1-exp(2/nrow(dat)*l0))
        prob = predict(lm_fit_full,type="response")
        auc = roc(dat$CAD, prob, quiet=T, plot=F)$auc
        auc
}
...
tiff("CAD1.tiff", units="in", width=6, height=4, res=300)
ggplot(res2, aes(x=method, y=R2)) + geom_boxplot(outlier.shape=NA, aes(fill=method)) + 
        geom_jitter(size=.5) + ggtitle("Coronary Artery Disease") +  
        ylab("AUC") + 
