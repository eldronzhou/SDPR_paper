...
pheno = read.table("../HDL/ukb29602.tab", header=T, stringsAsFactors=F)
bd = inner_join(bd, pheno, by="f.eid")

dat = inner_join(bd, SDPR, by=c("f.eid"="FID")) %>% 
        inner_join(pcs, by=c("f.eid"="FID"))

rm_idx = (is.na(dat$f.30870.0.0) | is.na(dat$f.22001.0.0) | is.na(dat$f.21003.0.0) | 
                  (dat$f.6177.0.0 != "None of the above") | is.na(dat$f.6177.0.0)) 
       
# mu = mean(dat$f.30870.0.0, na.rm=T); sigma = sd(dat$f.30870.0.0, na.rm=T)
# extreme_idx = (dat$f.30870.0.0 < mu-3*sigma) | (dat$f.30870.0.0 > mu+3*sigma)

dat = dat[!rm_idx, c("f.eid","f.30870.0.0","f.22001.0.0","f.21003.0.0",paste0("PC",1:10))]
colnames(dat) = c("ID","TG","sex","age", paste0("PC",1:10))

lm_fit_base = lm(TG~sex+age+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10, data=dat)
fid = (dat$ID)

get_cor = function(dat, i=-1) {
        if (i == -1)
                a = summary(lm(TG~sex+age+SCORE+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10, data=dat))$residuals
        else
                a = summary(lm(TG~sex+age+dat[,i]+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10, data=dat))$residuals
        b = summary(lm(TG~sex+age+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10, data=dat))$residuals
        1 - sum(a^2)/sum(b^2)
}
