...
rm_idx = (is.na(dat$f.30690.0.0) | is.na(dat$f.22001.0.0) | is.na(dat$f.21003.0.0) | 
...
dat = dat[!rm_idx, c("f.eid","f.30690.0.0","f.22001.0.0","f.21003.0.0",paste0("PC",1:10))]
colnames(dat) = c("ID","TC","sex","age", paste0("PC",1:10))
lm_fit_base = lm(TC~sex+age+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10, data=dat)
fid = (dat$ID)

get_cor = function(dat, i=-1) {
        if (i == -1)
                a = summary(lm(TC~sex+age+SCORE+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10, data=dat))$residuals
        else
                a = summary(lm(TC~sex+age+dat[,i]+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10, data=dat))$residuals
        b = summary(lm(TC~sex+age+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10, data=dat))$residuals
        1 - sum(a^2)/sum(b^2)
}
