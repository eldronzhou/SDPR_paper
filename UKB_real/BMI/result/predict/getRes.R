...
# gctb = read.table("gctb/dat.profile", header=T, stringsAsFactors=F)
...
rm_idx = is.na(dat$f.21001.0.0) | is.na(dat$f.22001.0.0) | is.na(dat$f.21003.0.0)
...
dat = dat[!rm_idx, c("f.eid","f.21001.0.0","f.22001.0.0","f.21003.0.0",paste0("PC",1:10))]
colnames(dat) = c("ID","BMI","sex","age", paste0("PC",1:10))
fid = (dat$ID)
BMI = dat

get_cor = function(dat, i=-1) {
        if (i == -1)
                a = summary(lm(BMI~sex+age+SCORE+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10, data=dat))$residuals
        else
                a = summary(lm(BMI~sex+age+dat[,i]+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10, data=dat))$residuals
        b = summary(lm(BMI~sex+age+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10, data=dat))$residuals
        1 - sum(a^2)/sum(b^2)
}
...
res = replicate(10, {
        ...
        # ldpred
        validate_dat = BMI[BMI$ID %in% validate_idx, ] %>%
        ...
        test_dat = BMI[BMI$ID %in% test_idx, ] %>%
                left_join(ldpred, by=c("ID"="FID"))
...
res_auto = replicate(10, {
        ...
        test_dat = BMI[dat$ID %in% test_idx, ] %>%
