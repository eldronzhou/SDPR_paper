...
rm_idx = (is.na(dat$f.30780.0.0) | is.na(dat$f.22001.0.0) | is.na(dat$f.21003.0.0) | 
...
dat = dat[!rm_idx, c("f.eid","f.30780.0.0","f.22001.0.0","f.21003.0.0",paste0("PC",1:10))]
colnames(dat) = c("ID","LDL","sex","age", paste0("PC",1:10))
lm_fit_base = lm(LDL~sex+age+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10, data=dat)
fid = (dat$ID)

get_cor = function(dat, i=-1) {
        if (i == -1)
                a = summary(lm(LDL~sex+age+SCORE+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10, data=dat))$residuals
        else
                a = summary(lm(LDL~sex+age+dat[,i]+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10, data=dat))$residuals
        b = summary(lm(LDL~sex+age+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10, data=dat))$residuals
        1 - sum(a^2)/sum(b^2)
}
...
tiff("LDL.tiff", units="in", width=6, height=4, res=300)
ggplot(res2, aes(x=method, y=R2)) + geom_boxplot(outlier.shape=NA, aes(fill=method)) + 
        geom_jitter(size=.5) + ggtitle("LDL") + 
